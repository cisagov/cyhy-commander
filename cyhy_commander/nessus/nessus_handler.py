...
BANNED_PLUGIN_IDS = [11219, #Nessus SYN scanner 
...
class NessusV2ContentHander(ContentHandler):
    ...
    def __init__(self, host_callback, report_callback, targets_callback, plugin_set_callback, port_range_callback, end_callback):
    ...
    def startElement(self, name, attrs):
        ...
        if len(self.modeStack) == 0:
            if name == 'NessusClientData_v2':
                ...
                raise SAXNotRecognizedException('XML does not look like Nessus v2 data.')
        else:
            mode = self.get_mode()
    ...
    def endElement(self, name):
        ...
        if mode == 'HostProperties' and name == 'tag':
            ...
            if tagName == 'HOST_START':
                tagName = 'start_time' 
                tagValue = datetime.strptime(tagValue, PROPS_DATE_FORMAT).replace(tzinfo=tz.tzutc())  # All times/dates assumed to be UTC
            elif tagName == 'HOST_END':
                tagName = 'end_time'
                tagValue = datetime.strptime(tagValue, PROPS_DATE_FORMAT).replace(tzinfo=tz.tzutc())
            else:
            ...
            if name.endswith('date'):
                tagValue = datetime.strptime(tagValue, REPORT_DATE_FORMAT).replace(tzinfo=tz.tzutc())
        ...
        elif name == 'HostProperties': #call back here so other reports will have an IP
            ...
            if name == 'name': 
                ...
                elif self.last_pref_name == 'plugin_set': # list of plugins separated by ;
                    self.plugin_set_callback(self.chars)
                elif self.last_pref_name == 'port_range': # list of port ranges separated by ,
