...
class NessusController(object):
    ...
    def __make_request(self, target, method, payload=None):
        ...
        while num_retries < FAILED_REQUEST_MAX_RETRIES:
            ...
            if self.token == None and target != LOGIN:
                ...
                self.__make_request(LOGIN, 'POST', {'username':self.user, 'password':self.password})
            ...
            if self.token and response.status_code == INVALID_CREDS_STATUS:
                ...
                LOGGER.warning('Attempting to establish new Nessus session (username: {!r})'.format(self.user))
    ...
    def policy_details(self, policy_id):
        response = self.__make_request(POLICY_DETAILS.format(policy_id=policy_id), 'GET')
    ...
    def policy_edit(self, policy_id, policy_details):
        response = self.__make_request(POLICY_EDIT.format(policy_id=policy_id), 'PUT', policy_details)
        if response.status_code == OK_STATUS:
            return response
        else:
    ...
    def policy_delete(self, policy_id):
        response = self.__make_request(POLICY_DELETE.format(policy_id=policy_id), 'DELETE')
        if response.status_code == OK_STATUS:
            return response
        else:
    ...
    def scan_details(self, scan_id):
        ...
        if response.status_code == OK_STATUS and response.json().get('info'):
            ...
            raise Warning('Scan id {!r} not found; response={!r}'.format(scan_id, response.text))
        else:
    ...
    def report_ready(self, scan_id, file_id):
        response = self.__make_request(REPORT_STATUS.format(scan_id=scan_id, file_id=file_id), 'GET')
    ...
    def report_download(self, scan_id):
        response = self.__make_request(REPORT_EXPORT.format(scan_id=scan_id), 'POST', {'format':'nessus'})
        if response.status_code == OK_STATUS and response.json().get('file'):
            ...
            response = self.__make_request(REPORT_DOWNLOAD.format(scan_id=scan_id, report_file_id=report_file_id), 'GET')
            if response.status_code == OK_STATUS:
                return response.text
            else:
                raise Warning('Scan report download failed; response={!r}'.format(response.text))
        else:
...
def main():
    ...
    assert len(possible_targets_files) > 0, 'No target file names matched: {!s}'.format(TARGET_FILE_GLOB)
    # pick the first one
    ...
    assert len(possible_ports_file) > 0, 'No ports file names matched: {!s}'.format(PORTS_FILE_GLOB)
    #pick the first one
    ...
    assert base_policy != None, 'Could not find policy {!r}'.format(controller.base_policy)
    ...
    assert new_policy['policy_id'] != base_policy['id'], 'New policy has the same id as the source policy: {policy_id}'.format(**new_policy)
    new_policy_id = new_policy['policy_id']
    LOGGER.info('Created new policy: \'{policy_name}\' (id: {policy_id})'.format(**new_policy))
    ...
    new_scan = controller.scan_new(targets_string, new_policy_id, new_scan_name, new_policy_details['uuid'])
    assert new_scan['scan']['uuid'], 'New scan was not created'
    new_scan_id = new_scan['scan']['id']
    LOGGER.info('Scan successfully created.  Name: \'{name}\'  id: {id}  UUID: {uuid}'.format(**new_scan['scan']))
    ...
    LOGGER.info('Scan launched successfully.  scan_uuid: "{scan_uuid}"'.format(**scan_launch_response))

    # wait for the scan to complete
    scan_status = controller.scan_status(new_scan_id)
    while scan_status in [SCAN_RUNNING_STATUS, SCAN_PROCESSING_STATUS]:
        LOGGER.info('Waiting for scan to complete (current status: {})'.format(scan_status))
        scan_found = True
        time.sleep(WAIT_TIME_SEC)
        scan_status = controller.scan_status(new_scan_id)
    ...
    if scan_details_response['info'].get('status') == SCAN_COMPLETED_STATUS:
        ...
        raise Warning('Scan id {!r} stopped running with status {!r}'.format(new_scan_id, scan_details_response['info'].get('status')))
