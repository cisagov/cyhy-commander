...
class NessusController(object):
    ...
    def policy_list(self):
        ...
        seq, status, d = self.__parse_reply_kv(response, 'contents/policies/policy', 'policyID', 'policyName')
        if self.__status_ok(status):
            return d
        else:
    ...
    def scan_new(self, targets, policy_id, scan_name):
        response = self.__make_request(SCAN_NEW, {'target':targets, 'policy_id':policy_id, 'scan_name':scan_name})
    ...
    def report_list(self):
        ...
        seq, status, v = self.__parse_reply_kv(response, 'contents/reports/report', 'name', 'status')
        if self.__status_ok(status):
            return v
        else:
...
def main():
    ...
    assert len(possible_targets_files) > 0, 'No target file names matched: %s' % TARGET_FILE_GLOB
    # pick the first one
    ...
    assert len(possible_ports_file) > 0, 'No ports file names matched: %s' % PORTS_FILE_GLOB
    #pick the first one    
    ...
    assert new_policy_id != policy_id, 'Policy copy has the same id as the source policy'
    ...
    assert result == new_policy_id, 'Expected policy delete to return the deleted policy id: %s != %s' % (result, policy_id)
