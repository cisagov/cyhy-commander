...
from cyhy.db import CHDatabase, IPPortTicketManager, IPTicketManager, DefaultHostStateManager
...
class NmapImporter(object):
    ...
    def __init__(self, db, stage=STAGE.PORTSCAN):
        if stage in (STAGE.NETSCAN1, STAGE.NETSCAN2):
            self.handler = NmapContentHander(self.__netscan_host_callback, self.__end_callback)
            self.__ticket_manager = IPTicketManager(db)
        elif stage == STAGE.PORTSCAN:
            self.handler = NmapContentHander(self.__portscan_host_callback, self.__end_callback)
            self.__ticket_manager = IPPortTicketManager(db, ['tcp']) #Nmap is only scanning TCP ports; don't close non-TCP ports.
            self.__ticket_manager.ports = xrange(1,65536) # A PORTSCAN is all ports.  Don't consider port 0 in scope.
        elif stage == STAGE.BASESCAN:
            self.handler = NmapContentHander(self.__baseline_host_callback, self.__end_callback)
    ...
    def __store_port_details(self, parsed_host):
        ...
        for (port,details) in parsed_host['ports'].items():
            if details['state'] != 'open': # only storing open ports
                continue
            if details.get('service', {}).get('name') == 'tcpwrapped':  # see https://secwiki.org/w/FAQ_tcpwrapped
                details['state'] = 'silent'     # tcpwrapped services are "silent", not really open ports
            else:
