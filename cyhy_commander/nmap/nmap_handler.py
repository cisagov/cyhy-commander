...
class NmapContentHander(ContentHandler):
    def __init__(self, host_callback, end_callback):
        ...
        self.first_osmatch_done_for_host = False # only capture the first osmatch element
    ...
    def startElement(self, name, attrs):
        ...
        if not self.isNmapFile:
            ...
            if attrs.has_key('starttime'):
                self.currentHost['starttime'] = datetime.datetime.utcfromtimestamp(int(attrs['starttime']))
                self.currentHost['endtime'] = datetime.datetime.utcfromtimestamp(int(attrs['endtime']))
            else:
            ...
            self.currentHost['hostname'] = attrs['name'] # can be multiple, only storing last
        elif name == 'port':
        ...
        elif name == 'osmatch' and self.xmloutputversion == '1.04' and not self.first_osmatch_done_for_host:
            os = self.currentHost['os'] = {'classes':[]}
            copy_attrs(attrs, os)
        elif name == 'osclass' and self.xmloutputversion == '1.04' and not self.first_osmatch_done_for_host:
            clazz = {}
            ...
            if attrs.has_key('time'):
                self.taskStartTime = datetime.datetime.utcfromtimestamp(int(attrs['time']))
        elif name == 'taskend':
            # save end time for hosts that don't have a time reported
            if attrs.has_key('time'):
                self.taskEndTime = datetime.datetime.utcfromtimestamp(int(attrs['time']))
