...
IPS_PER_NETSCAN1_JOB = 128 / 4
IPS_PER_NETSCAN2_JOB = 128 / 4
IPS_PER_PORTSCAN_JOB = 32 / 4
IPS_PER_VULNSCAN_JOB = 16
...
class Commander(object):
    ...
    def __setup_sources(self):
        if self.__test_mode:
            self.__nmap_sources.append(DatabaseJobSource(SLEEP_JOB_FILE, self.__db, job_type=STAGE.NETSCAN1, count=IPS_PER_NETSCAN1_JOB))
            self.__nmap_sources.append(DatabaseJobSource(SLEEP_JOB_FILE, self.__db, job_type=STAGE.NETSCAN2, count=IPS_PER_NETSCAN2_JOB))
            self.__nmap_sources.append(DatabaseJobSource(SLEEP_JOB_FILE, self.__db, job_type=STAGE.PORTSCAN, count=IPS_PER_PORTSCAN_JOB))
            self.__nessus_sources.append(DatabaseJobSource(SLEEP_JOB_FILE, self.__db, job_type=STAGE.VULNSCAN, count=IPS_PER_VULNSCAN_JOB))
            ...
            self.__nmap_sources.append(DatabaseJobSource(NETSCAN1_JOB_FILE, self.__db, job_type=STAGE.NETSCAN1, count=IPS_PER_NETSCAN1_JOB))
            self.__nmap_sources.append(DatabaseJobSource(NETSCAN2_JOB_FILE, self.__db, job_type=STAGE.NETSCAN2, count=IPS_PER_NETSCAN2_JOB))
            self.__nmap_sources.append(DatabaseJobSource(PORTSCAN_JOB_FILE, self.__db, job_type=STAGE.PORTSCAN, count=IPS_PER_PORTSCAN_JOB))
            self.__nessus_sources.append(DatabaseJobSource(VULNSCAN_JOB_FILE, self.__db, job_type=STAGE.VULNSCAN, count=IPS_PER_VULNSCAN_JOB))
    ...
    def __setup_sinks(self):
        if self.__test_mode:
            ...
            self.__success_sinks.extend((netscan1_sink, netscan2_sink, portscan_sink, vulnscan_sink))
    ...
    def __done_jobs(self):
        ...
        if output.failed:
            self.__logger.warning('Unable to get listing of "%s" on %s' % (DONE_DIR, env.host_string))
            output = ''
        ...
        for job in doneJobs:
            ...
            if DONE_FILE in jobContents:
                self.__logger.info('%s is ready for pickup on %s' % (job, env.host_string))
                ...
                if exitCode == '0':
                    ...
                    destDir = FAILED_DIR
                    self.__logger.warning('%s had a non-zero exit code: %s' % (job, exitCode))
                ...
                self.__logger.warning('%s is not ready for pickup on %s' % (job, env.host_string))

    @task
    def __running_job_count(self):
        output = run('ls %s' % RUNNING_DIR)
        if output.failed:
            self.__logger.warning('Unable to get listing of "%s" on %s' % (RUNNING_DIR, env.host_string))
            return None
    ...
    def __push_job(self, job_path):
        ...
        if len(paths.failed) == 0:
            self.__logger.info('%s was pushed successfully to %s' % (job_path, env.host_string))
    ...
    def __fill_hosts(self, counts, sources, workgroup_name, jobs_per_host):
        while True:
            ...
            if job_path == None:
                self.__logger.debug('Not enough work available to fill %s hosts' % workgroup_name)
    ...
    def handle_term(self, signal, frame):
        self.__logger.warning('Received signal %d.  Shutting down after this work cycle completes.' % signal)
        self.__is_running = False

    def __check_stop_file(self):
        if os.path.exists(STOP_FILE):
            self.__logger.warning('Stop file found.  Shutting down after this work cycle completes.')
            os.remove(STOP_FILE)
            self.__is_running = False
    ...
    def do_work(self):
        ...
        self.__shutdown_when_idle = config.getboolean(config_section, SHUTDOWN_WHEN_IDLE)
        self.__logger.info('Idle shutdown: %s', self.__shutdown_when_idle)
        self.__setup_sources()
        self.__setup_sinks()

        # pairs of hosts and job sources
        work_groups  = ((NMAP_WORKGROUP, nmap_hosts, self.__nmap_sources, jobs_per_nmap_host),
                        (NESSUS_WORKGROUP, nessus_hosts, self.__nessus_sources, jobs_per_nessus_host))
        ...
        while self.__is_running:
            try:
                ...
                self.__logger.debug('Checking remotes for completed jobs to download and process')
                ...
                self.__logger.debug('Checking for scheduled DONE hosts to mark WAITING.')
                ...
                if self.__is_running:
                    ...
                    if now < next_cycle_start_time:
                        ...
                        self.__logger.debug('Sleeping for %1.1f seconds.\n\n\n' % sleep_time)
                        time.sleep(sleep_time)
                    else:
                        self.__logger.debug('No time to sleep.  Last cycle took %1.1f seconds.\n\n\n' % (now - cycle_start_time))
                self.__check_stop_file()
